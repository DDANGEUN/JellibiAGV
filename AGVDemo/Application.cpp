...
void Application::AlarmBeepP()
{
    tone(s_Buzzer, NOTE_C4,300);
    delay(200);
    noTone(s_Buzzer);
    delay(100);
    tone(s_Buzzer, NOTE_C4,300);
...
void Application::ProcessRFIDRead()
{
    if (_reader.Read()) {
        ...
        if (_builder.BuildCmdList(strUid)) {
          Serial.println(__FUNCTION__);
          Serial.println("state 바꾸기");
            AppCommandList();
        } else {
            if (Serial) {
